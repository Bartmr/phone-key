...
class SshModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("sign") Coroutine { data: ByteArray ->
      sign(data)
    }
  ...
  private fun loadKey(): KeyStore.PrivateKeyEntry {
    val keyStore = KeyStore.getInstance("AndroidKeyStore")
    keyStore.load(null)

    val entry = keyStore.getEntry(KEY_ALIAS, null)
      ?: throw IllegalStateException("SSH key entry is null")

    require(entry is KeyStore.PrivateKeyEntry) {
      "Expected PrivateKeyEntry, got ${entry::class.simpleName}"
    }
    return entry
  }
  ...
  private suspend fun sign(data: ByteArray): Map<String, Any> {
    val entry = loadKey()
